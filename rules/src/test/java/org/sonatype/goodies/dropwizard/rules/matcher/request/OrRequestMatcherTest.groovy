  ...
  extends TestSupport
{
  ...
  void 'match hit'() {
    ...
    def underTest = new OrRequestMatcher([
        new PathRequestMatcher(new ContainsStringMatcher('qux', false)),
  ...
  void 'match order'() {
    ...
    def underTest = new OrRequestMatcher([ matcher1, matcher2 ])
  ...
  void 'match order short-circuit'() {
    ...
    when(matcher2.matches(request)).thenReturn(false)

    assert underTest.matches(request)

    verify(matcher1, times(1)).matches(request)
    verify(matcher2, times(0)).matches(any(HttpServletRequest.class))
    verifyNoMoreInteractions(matcher1, matcher2)
  }
}
