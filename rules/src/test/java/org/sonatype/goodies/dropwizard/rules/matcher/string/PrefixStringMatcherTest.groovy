...
/**
 * {@link PrefixStringMatcher} tests.
 */
class PrefixStringMatcherTest
  extends TestSupport
{
  @Test
  void 'case-sensitive'() {
    def underTest = new PrefixStringMatcher('foo', false)
  ...
  @Test
  void 'ignore-case'() {
    def underTest = new PrefixStringMatcher('foo', true)
    assert underTest.matches('foo bar baz')
    assert underTest.matches('FOO BAR BAZ')
    assert !underTest.matches('a b c foo bar baz')
    assert !underTest.matches('A B C FOO BAR BAZ')
